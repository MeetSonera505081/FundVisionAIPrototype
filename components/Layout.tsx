    ...
    <div className="min-h-screen flex bg-[#FFF5F0]">
      ...
      <aside className="w-64 bg-slate-900 text-slate-400 fixed h-full hidden lg:flex flex-col z-50">
        ...
        <div className="p-6 mt-auto">
          <button 
            onClick={onLogout}
            className="w-full flex items-center px-4 py-3 text-sm font-bold text-red-400 hover:text-red-300 hover:bg-red-500/10 rounded-xl transition-all"
          >
            <i className="fas fa-sign-out-alt w-6 text-lg"></i>
            <span>Logout</span>
          </button>
          <div className="bg-slate-800 rounded-2xl p-4 mt-4">
            ...
            <div className="flex justify-between items-center mb-1">
              <span className="text-sm font-bold text-white">{user.isPro ? 'Pro Plan' : 'Free Plan'}</span>
      ...
      <aside className={`fixed inset-y-0 left-0 w-72 bg-slate-900 text-slate-400 z-[70] transform transition-transform duration-300 lg:hidden flex flex-col ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <nav className="flex-1 px-4 mt-4 space-y-1">
          ...
          <button 
            onClick={onLogout}
            className="w-full flex items-center px-4 py-3 text-sm font-bold text-red-400 mt-4"
          >
            <i className="fas fa-sign-out-alt w-6 text-lg"></i>
            <span>Logout</span>
          </button>
          ...
          <div className="flex items-center space-x-4">
            ...
            <div className="flex items-center space-x-3 pl-2">
              <div className="text-right hidden sm:block">
                ...
                <p className="text-[10px] text-slate-500 mt-1">{user.isPro ? 'Pro Member' : 'Standard Member'}</p>
              </div>
              <img src={`https://picsum.photos/seed/${user.id}/40/40`} className="w-10 h-10 rounded-full ring-2 ring-orange-500 ring-offset-2" alt="Profile" />
            </div>
          </div>
        </header>
